...
if __name__ == "__main__":
    ...
    config = GAConfig()
    config.set_selection_method(SelectionMethod.BEST)
    
    selected = SelectionOperators.select_parents(population, config, 5)
    print(f"Selected {len(selected)} parents:")
    for i, parent in enumerate(selected):
        print(f"Parent {i}: fitness = {parent.fitness:.4f}")
    
    # Test tournament selection
    print("\nTesting Tournament Selection:")
    config.set_selection_method(SelectionMethod.TOURNAMENT, tournament_size=3)
    ...
    config.set_selection_method(SelectionMethod.ROULETTE_WHEEL, selection_pressure=2.0)
